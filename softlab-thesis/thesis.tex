...
in the presence of multiple evaluation orders (call-by-value, call-by-name, call-by-need). Our optimisation can be combined with  the 
defunctionalization transformation, so that the resulting performance optimisation also supports
...
Our optimisation can be combined with the
...
\emph{tail-call optimisation} can reuse it to construct the activation
record of the callee, saving space, and making some programs run in
constant space. Tail-calls can be further optimised by the
...
ones~\cite[\S{9}]{McCarthy62}\cite{Barron68}.  Tail-call optimisation
...
easy to compose with tail-call optimisation. Activation records may
...
may allocate activation records on the heap. Tail-call optimisation is
also important for implementations of logic languages~\cite{Bigot99}.

Tail-call optimisation in C: GCC~\cite{Probst01}, LLVM~\cite{Pandey:2015:LC:2842773}.

Tail-call optimisation for functional programming languages running on
