...
\begin{abstracten}%
  ...
  while the latter is a key feature of imperative programming languages. Although recursion may 
  ...
  optimization, a very simple but, as it turned out, very effective in order to eliminate the above 
  problem.
  In addition to Scheme, this optimization is included in most implementations of functional programming 
  ...
  optimization to eliminate memory overhead due to recursion.
  
  The purpose of this thesis is to integrate this well-known compiler optimization
  (tail call optimization) for strict functional programming languages in
  functional programming languages with multiple evaluation orders (call-by-value, call-by-name and
...
in the presence of multiple evaluation orders (call-by-value, call-by-name, call-by-need). More 
specifically our purpose is to combine this optimisation with a program transformation based on the 
...
to define control flow (structures) as abstaractions instead of primitives and to increase performance 
by avoiding needless calculations, and avoiding error conditions when evaluating compound expressions.
But everything comes at a cost: too lazy programs can lead to poor performance and even memory leaks. 
...
In this thesis, we get the best of both worlds: our programs are neither too lazy nor too strict,
while strictness provides the option to use tail-call optimisation in lazy evaluated 
functional programming languages and thus to eliminate memory overhead associated with recursion in the 
...
We combine this optimisation with a program representation based on the 
defunctionalization transformation, aiming to eliminate performance penalties associated with 
...
\begin{itemize}
  \item In \textit{Chapter 2}, we provide the necessary background for the reader in 
        order to follow the next chapters. There is a comprehensive explanation of classic 
        tail-call optimisation (section 2.1), an overview of the evaluation orders we study 
        in this thesis (section 2.2), an overview of static analysis (section 2.3). Finally, 
        in section 2.4 there is an overview of abstact machines and interpreters, focusing on 
        lazy functional languages (sections 2.4.2-2.4.4) as well as more general techniques 
        (section 2.4.5).

  \item In \textit{Chapter 3}, there is an overview of the thesis; we provide the intuition 
        for our key ideas presented in the rest of the thesis.

  \item In \textit{Chapter 4}, we present the language we studied. Briefly, we describe 
        how from a higher-order functional language and applying the appropriate 
        transformations (sections 4.2-4.4) we can reach the first-order functional 
        language we used for our analysis and evaluation of the optimisation.

  \item In \textit{Chapter 5} the execution model of our language exists. We present operational 
        properties as well as implementation details.

  \item In \textit{Chapter 6}, there is the static analysis algorithm that we use in order to spot 
        and thus properly annotate true tail-call positions. These annotated function calls 
        are later handled by the interpreter.

  \item In \textit{Chapter 7}, how we operationally treat inside the interpreter the 
        tail call optimisable function calls and the tail recursion modulo cons.

  \item In \textit{Chapter 8}, there is an evaluation of our optimisations on microbenchmarks.

  \item \textit{Chapter 9} and \textit{Chapter 10} are related work and future wok respectively.
...
for non-functional languages, as for example for C/C++~\cite{Probst01} 
or even Java~\cite{Madsen:2018:TCE:3178372.3179499}.
...
\begin{itemize}
  ...
  and the position inside the expression list in the constructor pattern. The `case' id is 
  unique inside a function definiton (locally).
\end{itemize}
...
the interpreter, on which we evaluate our technique for the tail-call optimisation and the 
...
\begin{itemize}
  \item We implement a first-order lazy abstract machine. First-order as we don't support
  ...
  abstract machine didn't provide support for that (section 2.4.3).
\end{itemize}
...
evaluation order (we use the abbrevation e.o. which stands for evaluation order). Later, we give the v' and s' for 
  ...
  \begin{itemize}
    \item cbv e.o.: 
      \begin{figure}[h]
        \[
          \mathit{(v', s') = (val, s), where \hfill\ v = StrictArg \hfill\ val}
        \]
...
\begin{itemize}
  ...
  \end{figure}
  As shown in figure~\ref{fig:makeCBVArg} we have \textit{(frameArg, state')}, where 
  \textit{frameArg} is added to \textit{[FrameArg]} and 
...
where:
\begin{itemize}
\item in section 7.1 we have the evaluation of tail-call optimisation, first expression 
      shown in figure~\ref{fig:annos},
\item and in section 7.2 we have the evaluation of tail recursion modulo cons shown
